/**
 * 流光 Liquid · app.js
 * Liquid Glass：底部胶囊导航 + 悬浮玻璃面板 + 镜面高光，mesh 幻彩背景。
...
(function () {
  ...
  const bgEl = el("div", { class: "liquid-bg", "aria-hidden": "true" },
    el("div", { class: "blob lg1" }), el("div", { class: "blob lg2" }),
    el("div", { class: "blob lg3" }), el("div", { class: "blob lg4" }),
    el("div", { class: "grain" }));
  ...
  function buildRail() {
    ...
    for (const item of NAV) {
      ...
      btn.innerHTML = icon(item.icon, 20);
      btn.append(el("span", { class: "rail-tip" }, SC.t(item.label)));
      if (item.badge) {
  ...
  if (SC.demo) {
    // 挂到 body：rail 有 backdrop-filter，会把 fixed 子元素变成相对自身定位
