...
import { Wallpaper, WallpaperMeta, WallpaperType, isPlaylist, getFileType } from "@/lib/client/types"
...
export function WallpaperDialog(props: WallpaperDialogProps) {
    ...
    const watchIsPlaylist = form.watch("wallpaperType") === WallpaperType.Playlist;
    ...
    useEffect(() => {
        if (!props.open) {
            ...
            const playlist = isPlaylist(props.wallpaper?.meta);
            if (props.wallpaper) {
                if (!playlist) {
                    setImportedFile({
        ...
        <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
                <DialogTitle>{props.wallpaper ?
                    `${watchIsPlaylist ? dictionary['local'].edit_list : dictionary['local'].edit_wallpaper}` :
                    `${watchIsPlaylist ? dictionary['local'].create_playlist : dictionary['local'].create_wallpaper}`}
                </DialogTitle>
                <DialogDescription>
                    {`${watchIsPlaylist ? dictionary['local'].local_list_description : dictionary['local'].local_wallpaper_description}`}
                </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
                <div className="flex w-full justify-between">
                    {
                        watchIsPlaylist ? <Button type="button" variant="secondary" onClick={() => { setOpenSelectWallpaperDialog(true) }}>
